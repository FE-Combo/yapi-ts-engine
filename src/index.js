...
async function generate(options) {
  try {
      ...
      if(!content?.data?.list) {
        ...
        for(item of content.data.list) {
          ...
          if(!response.errcode) {
            ...
            const apiName = `${stringFirstUpperCase(data.method.toLowerCase())}${(typeof apiRename === "function" ? apiRename(data.path) :data.path).split(/\-|\/|\_/).map(_=>stringFirstUpperCase(_)).join("").replace(/(\{)|(\}\{)/g, "With").replace(/\}$/g,"").replace(/\}/g,"In")}`
    ...
    console.info(chalk`{white.bold 😍 Generated Successfully (total: ${Object.keys(apiJSON).length})}`);
  } catch (e) {
